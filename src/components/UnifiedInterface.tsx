...
import type { Note, ChatInterfaceRef, NotesCaptureRef, ADKConfig, Job } from '../types';

type InterfaceMode = 'job' | 'notes';

export interface UnifiedInterfaceProps extends ADKConfig {
  /** Initial mode ('job' or 'notes') */
  initialMode?: InterfaceMode;
  /** Called when a job is extracted */
  onJobExtracted?: (job: Job) => void;
  /** Called when a note is saved */
  onNoteSaved?: (note: Note) => void;
  /** Custom logout handler */
  onLogout?: () => void | Promise<void>;
  /** Custom navigation to job board */
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-t-2xl shadow-lg p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              >
                <span
                  className={`inline-block h-10 w-10 transform rounded-full bg-white shadow-md transition-transform ${mode === 'notes' ? 'translate-x-12' : 'translate-x-1'
                    }`}
                >
          ...
          <div className="mt-4 p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg border border-gray-200">
            <div className="flex items-start space-x-3">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${mode === 'job' ? 'bg-blue-100' : 'bg-purple-100'
                }`}>
                {mode === 'job' ? (
                  <svg className="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex h-[700px]">
              ...
              <div className="flex-1 border-r border-gray-200">
                <ChatInterface
                  ...
                  onLogout={onLogout}
                  onNavigateToJobBoard={onNavigateToJobBoard}
                  onJobExtracted={onJobExtracted}
                />
              </div>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className={`p-6 rounded-xl border-2 transition-all ${mode === 'job'
          ...
          <div className={`p-6 rounded-xl border-2 transition-all ${mode === 'notes'
